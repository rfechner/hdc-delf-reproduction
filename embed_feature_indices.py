...
bxs = np.random.uniform(size=(nx + 1, d))
bys = np.random.uniform(size=(ny + 1, d))
feature_indices = np.random.uniform(size=(200, d)) # we have 200 features per image
...
def bind_bundle_batched(grp, kps, fs):
    ...
    for pos, f in zip(posit, fsit):
        ...
        tmp = pos * f * feature_indices
        tmp = tmp.sum(axis=1)
        outs.append(tmp)
    outs = np.concat(outs, axis=0)
    return outs 

def standartize_per_image(features, eps=1e-8):
    fmean, fstd = np.mean(features, axis=1, keepdims=True), np.std(features, axis=1, keepdims=True)
    features = (features - fmean) / (fstd + eps)
    return features
